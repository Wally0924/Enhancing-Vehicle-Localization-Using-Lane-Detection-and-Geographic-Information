...
class DistanceMatcher(BaseMatcher):
    ...
    def logprob_trans(self, prev_m, edge_m, edge_o,
        ...
        :return:
        """
        def cat_angle(self, node_1_coord, node_2_coord):

            vector_proj = [node_2_coord[0]-node_1_coord[0], node_2_coord[1]-node_1_coord[1]] 
            vector_proj = np.array(vector_proj)
            
            
            # # print(th, vector_gps, vector_proj)
            L_gps=np.sqrt(self.vector_gps.dot(self.vector_gps))
            L_proj=np.sqrt(vector_proj.dot(vector_proj))

            cos_angle=self.vector_gps.dot(vector_proj)/(L_gps*L_proj)
        ...
        if is_prev_ne or is_next_ne:
            ...
            beta = self.beta
        logprob = -d_t ** 2 / beta
        # print('dt', d_t, 'beta', beta, 'logprob', logprob)
        # Penalties
        ...
        if self.vector_gps is not None:
            angle = cat_angle(self, self.map.node_coordinates(edge_m.l1), self.map.node_coordinates(edge_m.l2))
            if angle > 30:
                logprob += self.angle_log
        props = {
            'd_o': d_z,
            'd_s': d_x,
            'lpt': logprob
